...
no_merged <- bind_rows(no_2016, no_2021)

no_inter <- st_intersection(no_2016, no_2021) %>%
  dplyr::select(NUTS_ID, NUTS_ID.1) %>%
  st_make_valid(.) %>%
  st_collection_extract("POLYGON")

for(i in 1:nrow(no_inter)){
  no_inter$area[i] <- st_area(no_inter[i,])
}
no_inter <- no_inter %>%
  arrange(desc(area)) %>%
  slice(1:11) %>%
  arrange(NUTS_ID) %>%
  mutate(nuts_flow = paste0(NUTS_ID, "→", NUTS_ID.1)) %>%
  st_make_valid(.)

pop <- raster("JRC_1K_POP_2018.tif")
no_inter <- no_inter %>% st_transform(crs(pop))
no_2016_1 <- no_2016 %>% st_transform(crs(pop))
no_2021_1 <- no_2021 %>% st_transform(crs(pop))
no_pop <- crop(x = pop, y = as_Spatial(no_inter))
no_pop <- mask(no_pop, as_Spatial(no_inter))

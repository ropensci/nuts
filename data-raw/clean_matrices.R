...
csv_files = list.files("data-raw/nuts-matrices/", pattern="*.csv", full.names=TRUE)
...
for( i in seq_along(csv_files) ){
  ...
  t <- t %>%
    ...
    select( from_code , to_code
            ...
            , areaKm , pop18 , pop11 , artif_surf18 , artif_surf12
             )
  ...
  t3_a <- t %>%
    ...
    summarise_at( vars( areaKm : artif_surf12 ) , ~sum( . )) %>%
    ungroup( ) %>%
    unique( ) %>%
